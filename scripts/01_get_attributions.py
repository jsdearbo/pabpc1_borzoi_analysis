...
def main():
    ...
    centering_mode = cfg.get(
        "centering_mode", cfg.get("attr_respect_to", "element_only")
    )
    ...
    genome = (
        ...
        else ("hg38" if species == "human" else "mm10")
    )
    ...
    if model_selection == "fine_tuned":
        ...
        tasks_list = tasks[tasks["name"].str.contains(task_id, na=False)][
            "name"
        ].tolist()
    ...
    for item in element_inputs:
        try:
            ...
            selected_bins = get_eval_bins(
                model, item.input_intervals, item.eval_intervals
            )
    ...
    mapping_df.to_csv(
        os.path.join(experiment_dir, "attribution_mapping.csv"), index=False
    )
