...
def make_input_interval(
    chrom: str, start: int, end: int, seq_len: int, strand: str
) -> pd.DataFrame:
...
def fetch_sequence(
    intervals: pd.DataFrame, species: str, genome_version: Optional[str] = None
) -> str:
    ...
    if species == "human":
        ...
        if genome_version == "GRCm39":
            ivals["chrom"] = ivals["chrom"].apply(
                lambda x: (
                    str(x).replace("chr", "") if str(x).startswith("chr") else str(x)
                )
            )
...
def prepare_inputs(
    ...
    for idx, row in df.iterrows():
        ...
        inputs.append(
            AttributionInput(sequence, input_intervals, eval_intervals, name, meta=row)
        )
            ...
            pd.DataFrame(
                {
                    ...
                    "transcript_name": exon_group["transcript_name"]
                    .iloc[:num_introns]
                    .values,
                    "transcript_id": exon_group["transcript_id"]
                    .iloc[:num_introns]
                    .values,
...
def create_elements_dataframe(
    intron_df: pd.DataFrame, feature_df: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    for transcript_id in feature_df["transcript_id"].dropna().unique():
        ...
        transcript_introns = intron_df[
            intron_df["transcript_id"] == transcript_id
        ].copy()
        ...
        if (
            "exon_number" in transcript_exons.columns
            and transcript_exons["exon_number"].notna().any()
        ):
            ...
            transcript_exons["element"] = "exon_" + transcript_exons[
                "element_number"
            ].astype(str)
        ...
        if not transcript_utrs.empty:
            ...
            transcript_utrs = transcript_utrs.assign(
                element=lambda x: (
                    x["feature"].str.replace("_", "").str.lower()
                    + "_"
...
def attribution_native_only(
    attrs: np.ndarray, seq: str, on_unknown: str = "zero"
) -> np.ndarray:
    """
...
def adjacent_rows_transcriptional(
    elements_df: pd.DataFrame, transcript_name: str, intron_number: int, strand: str
):
    """
    ...
    for i, row in coords.iterrows():
        ...
        if mode in ["element_only", "intron_only", "peak_only"]:
            ...
            if "unique_ID" not in row:
                raise ValueError(f"Mode '{mode}' requires 'unique_ID' in coord data.")
            ...
            if mode == "upstream_exon":
                ...
                lo, hi = _clamp(
                    (intron_5p_idx - up_len) - flank, (intron_3p_idx + dn_len) + flank
                )
                if lo is None:
                    attributions[i, :, :] = masked_value
                else:
...
def remove_fasta_overlaps(
    primary_fasta: str, ctrl_fasta: str, cosi_group: str = "primary"
) -> str:
    """
    ...
    filtered = {h: s for h, s in ctrl_sequences.items() if h not in primary_sequences}
