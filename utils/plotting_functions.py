...
def plot_slices(
    ...
    for results in mutation_tasks:
        for slice_num, row in slices_df.iterrows():
            ...
            logger.debug(
                f"Plotting slice {slice_num} from {start_pos} to {end_pos} for result {results}"
            )
            if start_pos < 0 or end_pos >= ism_results[results].shape[1]:
                logger.error(
                    f"Invalid start or end position for slice {slice_num}: start_pos={start_pos}, end_pos={end_pos}"
                )
                continue
            for method, dir_, figsize in [
                ("heatmap", heatmap_dir, (20, 1.5)),
                ("logo", logo_dir, (15.5, 1.5)),
            ]:
                ...
                plt.title(
                    f"Track: {tasks['sample'].loc[results]} slice {slice_num}",
                    fontsize=14,
                )
                plt.xlabel(
                ...
                save_path = os.path.join(
                    dir_,
                    f"{elements_to_highlight}_slice_{slice_num}_track_{results}.png",
                )
...
def plot_full_window(
    ...
    for results in mutation_tasks:
        ...
        save_path = os.path.join(
            heatmap_dir, f"{elements_to_highlight}_full_track_{results}.png"
        )
        plt.savefig(save_path, bbox_inches="tight")
        plt.close()
...
def _plot_gene_elements(
    ...
    for element in elements_list:
        ...
        if "intron" in element_name:
            plot_start = (
                max(element.start, start) if start is not None else element.start
            )
            plot_end = min(element.end, end) if end is not None else element.end
            if element.end < (
                start if start is not None else element.start
            ) or element.start > (end if end is not None else element.end):
                continue
    ...
    for exon in elements_list:
        ...
        if "exon" in exon_name and "utr" not in exon_name:
            ...
            if exon.end < (start if start is not None else exon.start) or exon.start > (
                end if end is not None else exon.end
            ):
                continue
            ...
            overlapping_utrs = [
                ...
                if (
                    "utr" in str(getattr(utr, "element_type", "")).lower()
                    or "utr" in str(utr.element).lower()
                )
    ...
    for utr in elements_list:
        ...
        if "utr" in utr_type or "utr" in utr_name:
            ...
            if utr.end < (start if start is not None else utr.start) or utr.start > (
                end if end is not None else utr.end
            ):
                continue
            rect_height = 0.5
            rect_y = -rect_height / 2
...
def plot_gene_map(
    ...
    if highlight_area is not None:
        ...
        x_axis_label = (
            f"{chrom}:{fmt(start)}-{fmt(end)} | Highlight: {fmt(left)}-{fmt(right)}"
        )
    else:
        left = right = None
    ...
    _plot_gene_elements(
        ...
        color_exon=color_exon,
        color_intron=color_intron,
        exon_text_color=exon_text_color,
        intron_text_color=intron_text_color,
    )
...
def plot_logo_and_optional_gene_map(
    ...
    if elements_df is not None and indexing_df is not None:
        ...
        intron_start = indexing_df.loc[
            indexing_df["unique_ID"] == seq_name, "start"
        ].values[0]
        intron_end = indexing_df.loc[
            indexing_df["unique_ID"] == seq_name, "end"
        ].values[0]
        ...
        fig, axs = plt.subplots(
            2, 1, figsize=(20, 6), gridspec_kw={"height_ratios": [2, 1]}
        )

        # Logo plot (top)
        plot_logo(
            arr, ax=axs[0], start=plt_start, end=plt_end, annotations=seq_annotations
        )
        axs[0].set_title(
            f"Motif: {getattr(row, 'pattern_label', getattr(row, 'motif_name', 'unknown'))} | Seq: {seq_name} | Score: {row.score:.2f}"
        )
        ...
        plt.close(fig)
        print(
            f"Logo+gene map plot saved for {seq_name} at: \n{logo_plot_dir}/logo_and_map_{seq_name}.png"
        )
    else:
        # Only logo plot
...
def plot_cosi_boxplot(
    ...
    sns.boxplot(
        x=labels, y=flattened_data, palette=palette, ax=ax, showfliers=False
    )  # Disable default outliers
...
def get_transcript_and_highlight_bounds(
    elements_df, seq_name, row_start, row_end, tensor_center=262144
):
    """
...
def _plot_read_density_tracks(
    ...
    for i, (bw_key, color) in enumerate(zip(timepoints, colors)):
        ...
        try:
            ...
            print(
                f"Error fetching values for gene {gene_label if gene_label else chrom} in {bw_key}: {e}"
            )
            bw.close()
            continue
        ...
        if highlight_spans is not None:
            ...
            if (
                isinstance(highlight_spans, list)
                and len(highlight_spans) > 0
                and isinstance(highlight_spans[0], tuple)
            ):
...
def _select_elements_for_sequence(
    elements_df: pd.DataFrame, seq_name: str
) -> pd.DataFrame:
    """
    ...
    if "transcript_name" in elements_df.columns:
        ...
        if not subset.empty:
            logging.info(
                f"[select_elements] Strategy A matched transcript_name '{base}' ({len(subset)} rows)"
            )
            return subset
    ...
    if "transcript_name" in elements_df.columns:
        subset = elements_df[
            elements_df["transcript_name"].str.startswith(base, na=False)
        ].copy()
        if not subset.empty:
            logging.info(
                f"[select_elements] Strategy A2 matched transcript_name startswith '{base}' ({len(subset)} rows)"
            )
            return subset
    ...
    if "gene_name" in elements_df.columns:
        ...
        if not subset.empty:
            logging.info(
                f"[select_elements] Strategy B matched gene_name '{base}' ({len(subset)} rows)"
            )
            return subset
    ...
    if "gene_name" in elements_df.columns:
        subset = elements_df[
            elements_df["gene_name"].str.contains(base, na=False)
        ].copy()
        if not subset.empty:
            logging.info(
                f"[select_elements] Strategy C matched gene_name contains '{base}' ({len(subset)} rows)"
            )
            return subset
...
def plot_gene_map_and_read_densities(
    ...
    if start_region_name is not None:
        ...
        if start_row.empty:
            raise ValueError(
                f"Start region '{start_region_name}' not found in elements_df for transcript {tscript_name}"
            )
        plot_start = (
            int(start_row["end"].iloc[0])
            if strand == "-"
            else int(start_row["start"].iloc[0])
        )
    else:
    ...
    if end_region_name is not None:
        ...
        if end_row.empty:
            raise ValueError(
                f"End region '{end_region_name}' not found in elements_df for transcript {tscript_name}"
            )
        plot_end = (
            int(end_row["start"].iloc[0])
            if strand == "-"
            else int(end_row["end"].iloc[0])
        )
    else:
    ...
    if axes is None:
        ...
        if len(axes) != n_tracks + 1:
            raise ValueError(
                f"Number of axes ({len(axes)}) does not match number of tracks + 1 ({n_tracks + 1})"
            )
        fig = axes[0].figure

    # Subset elements_df to only those overlapping the region
    elements_df_sub = elements_df[
        (elements_df["end"] >= plot_start) & (elements_df["start"] <= plot_end)
    ].copy()
    plot_gene_map(
    ...
    if track_highlight is not None:
        if isinstance(track_highlight, list):
            ...
            intron_start = elements_df.loc[
                elements_df["element"] == track_highlight, "start"
            ].values[0]
            intron_end = elements_df.loc[
                elements_df["element"] == track_highlight, "end"
            ].values[0]
            if flip_x:
                highlight_left, highlight_right = max(intron_start, intron_end), min(
                    intron_start, intron_end
                )
            else:
                highlight_left, highlight_right = min(intron_start, intron_end), max(
                    intron_start, intron_end
                )
    ...
    for i, (bw_key, ax_rd) in enumerate(zip(timepoints, axes[1:])):
        ...
        try:
            ...
            print(
                f"Error fetching values for gene {gene if gene else chrom} in {bw_key}: {e}"
            )
            bw.close()
            continue
        if y_scale == "log":
        ...
        ax_rd.fill_between(
            x_range, values, color=plt.cm.viridis(i / n_tracks), alpha=0.3
        )
    ...
    if axes is None:
        fig.suptitle(
            f"Gene Map and Signal Intensity for {gene} at Each Time Point in {y_scale} scale"
        )
...
def plot_logo_gene_map_and_read_densities(
    ...
    if seq_elements_df.empty:
        ...
        seq_elements_df = pd.DataFrame(
            columns=["chrom", "start", "end", "strand", "transcript_name"]
        )
        ...
        gene_name = (
            seq_elements_df["gene_name"].iloc[0]
            if "gene_name" in seq_elements_df.columns
    ...
    if seq_name in indexing_df["unique_ID"].values:
        ...
        if "element_start" in row_idx and pd.notna(row_idx["element_start"]):
            ...
            center_ref_point = (
                int(row_idx["element_start"]) + int(row_idx["element_end"])
            ) // 2
        else:
    ...
    if n_tracks == 0:
        fig = plt.figure(
            figsize=(figsize[0], max(figsize[1], 6.0)), constrained_layout=True
        )
        ...
        fig = plt.figure(
            figsize=(figsize[0], figsize[1] + 2 * n_tracks), constrained_layout=True
        )
    ...
    print(
        f"attribution array shape: {arr.shape}, plotting from {plt_start} to {plt_end}"
    )
    ...
    if arr_np.shape[0] == 4:
        ...
        raise ValueError(
            f"Attributions must have a channel axis of 4, got {arr_np.shape}"
        )
    ...
    if plt_end <= plt_start:
        raise ValueError(
            f"Empty logo window for {seq_name}: start={plt_start}, end={plt_end}"
        )
    ...
    print(
        f"attribution array shape for logo: {arr_logo.shape}, plotting from {plt_start} to {plt_end}"
    )

    plot_logo(
        arr_logo, ax=ax_logo, start=plt_start, end=plt_end, annotations=seq_annotations
    )
    ax_logo.set_title(
        f"Motif: {getattr(row, 'pattern_label', getattr(row, 'motif_name', 'unknown'))} | Seq: {seq_name}"
    )
    ax_logo.set_ylabel("Attribution Score")
    if n_tracks == 0:
        ax_logo.set_xlabel("")
    else:
        ax_logo.set_xlabel("Position")
    ...
    if n_tracks > 0 and not seq_elements_df.empty:
        ...
        if element_highlight_bounds:
            ...
            if flip_x:
                highlight_spans = (
                    min(highlight_left, highlight_right),
                    max(highlight_left, highlight_right),
                )
            else:
                highlight_spans = (
                    min(highlight_left, highlight_right),
